...
int usb_lpbk_get_desc(struct usb_gadget *gadget, const usb_device_request_t *ctrl)
{
    ...
    switch (desc_type) {
        ...
        default:
            ...
            LOGE(__func__, "Unsupported desc_type %d", desc_type);
            return -ENOTSUPP;
    }
...
struct usb_ep *usb_lpbk_get_bulk_in_ep(struct usb_gadget *gadget)
{
    return gadget_find_eq_by_name(gadget, "ep1in");
}

struct usb_ep *usb_lpbk_get_bulk_out_ep(struct usb_gadget *gadget)
{
...
int usb_lpbk_enable_eps(struct usb_gadget *gadget, f_loopback_t *lpbk)
{
    ...
    lpbk->ep_in  = usb_lpbk_get_bulk_in_ep(gadget);
    configASSERT(lpbk->ep_in != NULL);
    lpbk->ep_out = usb_lpbk_get_bulk_out_ep(gadget);
    ...
    if (ret < 0) {
        ...
        usb_lpbk_disable_eps(gadget, lpbk);
        return ret;
    }
