...
static int ov02b_write_reg(int i2c_num, uint32_t i2c_tar_addr, uint16_t reg_addr, uint8_t reg_val)
{
    int ret;
    uint8_t data_buf[2];
    data_buf[0] = reg_addr;//j将寄存器的地址放入缓存区的第一位
    data_buf[1] = reg_val;//将寄存器的值放入缓存区的第二位
    ret = i2c_send_data(i2c_num, i2c_tar_addr, data_buf, 2);  //发送数据
...
static int ov02b_read_reg(int i2c_num, uint32_t i2c_tar_addr, uint16_t reg_addr, uint8_t *reg_val)
{
    ...
    ret = i2c_send_data(i2c_num, i2c_tar_addr, (const uint8_t *)&reg_addr, 1);//发送要读的地址
    /*ret = i2c_send_data_dma(DMAC_CHANNEL0, i2c_num, &reg_addr, 1);*/
    if (ret < 0) {
        return ret;
    }
    ret = i2c_recv_data(i2c_num, i2c_tar_addr, 0, 0, reg_val, 1);//接收寄存器的值
...
static int ov02b_program_regs(
        int             i2c_num, //I2C总线编号
		uint32_t 		i2c_tar_addr,//I2C目标设备地址
        const OV02B_REG_T    *reg_list,//要编程的寄存器列表
        int             cnt// 寄存器列表中的寄存器数量
        )
{
    int i;
    int ret = 0;
