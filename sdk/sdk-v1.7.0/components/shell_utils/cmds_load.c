...
int serial_load(int argc, char **argv)
{
    ...
    flash_info.clk_rate      = 10 * 1000 * 1000;
    ...
    if (mode != LOAD_TO_MEM) {
        ...
        if (!is_compressed_file(filename)) {
            ...
            wr_bytes = rd_size;
#else
            shellPrint(shell, "gzip is not supported\n");
            ret = -1;
            goto out;
#endif
        }
...
int serial_load_file(const char *filename)
{
    ...
    crc_recv = crc32(0, (uint8_t*)recv_buf, recv_bytes);
    ...
    ret = fs_file_read(fp, recv_buf, recv_bytes);
    if (ret != recv_bytes) {
    ...
    crc_golden = crc32(0, (uint8_t*)recv_buf, recv_bytes);

    if (crc_recv == crc_golden) {
        shellPrint(shell, "Verify end: SUCCESS!\n");
    }
    else {
        shellPrint(shell, "Verify end: FAIL!\n");
...
static int _serial_load_part(char *part_name, uart_device_number_t uart_num, int uart_baudr)
{
    ...
    recv_bytes = ota_load_ymodem(chan, cfg.dev, cfg.baudr, 
            flash_info.start_rw_addr, LOAD_AND_DECOMPRESS_TO_FLASH, part_hdr.size, filename);
    userShellResume();
    if (recv_bytes == 0) {
        ret = -1;
        goto out;
