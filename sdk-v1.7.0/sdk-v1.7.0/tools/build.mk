...
TARGET_FW_ADDR   := 0x80800000

TARGET_FW_MARKER  := 0x61697661

ifeq ($(CONFIG_EXECUTE_ON_DDR),y)
	ifeq ($(CONFIG_XIP_EXEC_DEBUG),y)
		TARGET_FW_ADDR   := 0x80800000
	else
		ifeq ($(CONFIG_XNN_PING_PONG_BUF),y)
			ifeq ($(CONFIG_CHIP_AI1102B),y)
				TARGET_FW_ADDR   := 0x80500000
			else ifeq ($(CONFIG_CHIP_AI1103B),y)
				TARGET_FW_ADDR   := 0x80500000
			else ifeq ($(CONFIG_CHIP_AI1102C),y)
				TARGET_FW_ADDR   := 0x80500000
			else ifeq ($(CONFIG_CHIP_AI1103C),y)
				TARGET_FW_ADDR   := 0x80500000
			else
				TARGET_FW_ADDR   := 0x80800000
			endif # ifeq ($(CONFIG_CHIP_AI1102B),y)
		else
			ifeq ($(CONFIG_CHIP_AI1102B),y)
				TARGET_FW_ADDR   := 0x80400000
			else ifeq ($(CONFIG_CHIP_AI1103B),y)
				TARGET_FW_ADDR   := 0x80400000
			else ifeq ($(CONFIG_CHIP_AI1102C),y)
				TARGET_FW_ADDR   := 0x80400000
		...
		endif # ifeq ($(CONFIG_XNN_PING_PONG_BUF),y)
	endif # ifeq ($(CONFIG_XIP_EXEC_DEBUG),y)
endif # ifeq ($(CONFIG_EXECUTE_ON_DDR),y)

ifeq ($(CONFIG_EXECUTE_ON_ROM),y)
	# CONFIG_XIP_BOOT_ADDR=0x30010000
	TARGET_FW_ADDR   := $(CONFIG_XIP_BOOT_ADDR)
	CFLAGS += -DCONFIG_XIP_BOOT_ADDR=$(CONFIG_XIP_BOOT_ADDR)
endif

ifeq ($(CONFIG_BUILD_PUBLIC_VERSION),y)
	TARGET_FW_MARKER  := 0x6d617273
endif
...
$(LDSCRIPT): $(LDSCRIPT_SOURCE)
	echo "Generate ld file"
	mkdir -p $(BIN_DIR)
	$(CC) $(INC_LIB_DIR) $(CFLAGS) -E -P $^  > $@

$(BIN_DIR)/$(TARGET_LIB): $(OBJ_DIR) $(BIN_DIR) $(LIB_OBJS) $(LIBSDK) $(DEPEND)
	$(AR) r $(BIN_DIR)/$(TARGET_LIB) $(LIB_OBJS)


$(BIN_DIR)/$(TARGET_ELF): $(BIN_DIR) $(OBJ_DIR) $(APP_OBJS) $(BIN_DIR)/$(TARGET_LIB) $(LIBSDK) $(DEPEND) $(LDSCRIPT)
	@echo Linking $@
	@echo "$(APP_OBJS)"
...
$(BIN_DIR)/$(TARGET_IMG): $(BIN_DIR)/$(TARGET_BIN)
	@echo "$(BIN2IMAGE) -v $(TARGET_FW_VER) -i $(BIN_DIR)/$(TARGET_BIN) -s $(TARGET_FW_ADDR) -o $@ -f 0 -b $(TARGET_FW_BAUDR) -d $(TARGET_FW_DMAEN) -e $(TARGET_FW_EXC_EN) -p $(TARGET_FW_PLL_BP) -r $(TARGET_FW_RX_DLY) -t $(CONFIG_BOOT_DELAY_SECS) -m $(TARGET_FW_MARKER)"
	$(BIN2IMAGE) -v $(TARGET_FW_VER) -i $(BIN_DIR)/$(TARGET_BIN) -s $(TARGET_FW_ADDR) -o $@ -f 0 -b $(TARGET_FW_BAUDR) -d $(TARGET_FW_DMAEN) -e $(TARGET_FW_EXC_EN) -p $(TARGET_FW_PLL_BP) -r $(TARGET_FW_RX_DLY) -t $(CONFIG_BOOT_DELAY_SECS) -m $(TARGET_FW_MARKER)
...
# Rules
################################################
get_file_name = $(subst aiva_,,$(notdir $(1)))

define cmd-compile-app-c
$(OBJ_DIR)/$(notdir $(1:.c=.o)): $(MODULES) $(1) Makefile
	@echo Building $(1)
	$(CC) $(CFLAGS) -D__FILENAME__=\"$(call get_file_name, $(1))\" $(WERROR) $(INC_APP_DIR) $(INC_SDK_DIR) $(INC_LIB_DIR) -c -o $(OBJ_DIR)/$(notdir $(1:.c=.o)) $(1)
endef

define cmd-compile-app-cpp
$(OBJ_DIR)/$(notdir $(1:.cpp=.o)): $(MODULES) $(1) Makefile
	@echo Building $(1)
	$(CPPC) $(CFLAGS) -D__FILENAME__=\"$(call get_file_name, $(1))\" $(CXXFLAGS) $(INC_APP_DIR) $(INC_SDK_DIR) $(INC_LIB_DIR) -c -o $(OBJ_DIR)/$(notdir $(1:.cpp=.o)) $(1)
endef

define cmd-compile-lib-s
$(OBJ_DIR)/$(notdir $(1:.S=.o)): $(MODULES) $(1) Makefile
	@$(MAKENOISE) Building $(1)
	$(CC) $(CFLAGS) -D__FILENAME__=\"$(call get_file_name, $(1))\" $(INC_LIB_DIR) -c -O2 -o $(OBJ_DIR)/$(notdir $(1:.S=.o)) $(1)
endef

define cmd-compile-lib-c
$(OBJ_DIR)/$(notdir $(1:.c=.o)): $(MODULES) $(1) Makefile
	@echo Building $(1)
	$(CC) $(CFLAGS) -D__FILENAME__=\"$(call get_file_name, $(1))\" $(WERROR) $(INC_SDK_DIR) $(INC_LIB_DIR) -c -o $(OBJ_DIR)/$(notdir $(1:.c=.o)) $(1)
endef

define cmd-compile-lib-cpp
$(OBJ_DIR)/$(notdir $(1:.cpp=.o)): $(MODULES) $(1) Makefile
	@echo Building $(1)
	$(CPPC) $(CFLAGS) -D__FILENAME__=\"$(call get_file_name, $(1))\" $(CXXFLAGS) $(INC_SDK_DIR) $(INC_LIB_DIR) -c -o $(OBJ_DIR)/$(notdir $(1:.cpp=.o)) $(1)
endef
