...
static struct {
    ...
    {                       /* wString[] */
        ...
        UCONSTW('A'), UCONSTW('L'), UCONSTW('I'), UCONSTW('E'),
        UCONSTW('N'), UCONSTW('T'), UCONSTW('E'), UCONSTW('C'),
    },
};
...
static struct {
    ...
    {                       /* wString[] */
        // UCONSTW('A'), UCONSTW('I'), UCONSTW('V'), UCONSTW('A'),
        UCONSTW('D'), UCONSTW('W'), UCONSTW('C'), UCONSTW('3'),
...
static int uvc_isoc_cam_update_yuyv_frame_params(int w, int h, uint8_t index)
{
    if ((w<=0 || w > 2000) || (h<=0 || h>2000)) {
        LOGE(__func__, " frame param INVALID.");
        LOGE(__func__, " frame. w:%d,h:%d.", w, h);
        return -1;
    }

    if (index > 4) {
        LOGE(__func__, " frame param index INVALID.");
        LOGE(__func__, " frame param index:%d.", index);
        return -2;
    }

    struct UVC_FRAME_UNCOMPRESSED(3) frame = VS_UNCOMP_FRAME_DESC(index+1, w,  h,  w*h*2);
    hs_yuyv_config_desc.vs_frames_formats_desc.vs_uncomp_frames_desc_1[index] = frame;

    return 0;
}

int uvc_isoc_cam_set_yuyv_frame_desc(usb_ext4_frame_resolution_t *uvc_frame_resolution, int ext4_frame_resolution_num)
{
    int ret = 0;

    if (uvc_frame_resolution == NULL) {
        LOGE(__func__, " INVALIDE param.");
        return -1;
    }
    
    for (int i = 0; i < ext4_frame_resolution_num; ++i) {
        ret = uvc_isoc_cam_update_yuyv_frame_params(uvc_frame_resolution[i].width, uvc_frame_resolution[i].height, i);
        if (ret != 0) {
            LOGE(__func__, " update ext4 frame param failed and exit.");
        }
    }
